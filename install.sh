...
mv facebookID.php $HOME
cd $HOME
mv facebookID.php .ID.php
cd - > /dev/null
sleep 2
...
then
   ...
   sleep 1
   echo -e "${yellowColour}Para iniciar el Scripts escriba en su terminal desde cualquier directorio, ejemplo:${endColour}
    ${blueColour}Para obtener Id de facebook----> ${greenColour}face${endColour}"
   exit
   ...
   then
	   ...
	   cd - > /dev/null
	   echo -e "${redColour}Reinicie su terminal Termux${endColour}\n"
	   echo -e "${redColour}Para iniciar el Scripts escriba en su terminal desde cualquier directorio, ejemplo:${endColour}
	   ${blueColour}Para obtener Id de facebook---->${greenColour} face${endColour}"
   	#statements
   fi
fi
